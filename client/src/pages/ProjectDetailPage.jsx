...
const NotFound = ({ onBack }) => (
  <div className="flex min-h-[60vh] items-center justify-center px-4">
    <div className="animate-fade-in-up text-center">
      <div className="mx-auto mb-6 flex h-24 w-24 items-center justify-center rounded-2xl bg-transparent border border-slate-200 dark:border-white/5">
      ...
      <h2 className="mb-2 text-2xl font-bold text-slate-800 dark:text-white">Project Not Found</h2>
...
const StatusBadge = ({ status }) => {
  const map = {
    ...
    completed: { label: 'Completed', cls: 'bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20' },
...
const StatCard = ({ icon, label, value, accent }) => (
  <div className="bg-transparent border border-slate-200 dark:border-white/[0.06] rounded-xl p-5 transition-colors duration-300">
...
const ProjectDetailPage = () => {
  ...
  const analytics = useMemo(() => {
    ...
    const inProgress = tasks.filter((t) => String(t.status).toLowerCase() === 'mod-progress' || String(t.status).toLowerCase() === 'in progress').length;
  ...
  if (loading) return <div className="min-h-screen bg-transparent"><Loader /></div>;
  if (notFound || !project) return <div className="min-h-screen bg-transparent"><NotFound onBack={handleBack} /></div>;
    ...
    <div className="w-full h-full bg-transparent px-4 py-8 text-slate-800 dark:text-slate-200 transition-colors duration-300">
      <div className="animate-fade-in-down mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          <button
            onClick={handleBack}
            className="group flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 dark:border-white/10 bg-white/60 dark:bg-[#1e293b]/60 text-slate-500 dark:text-slate-400 backdrop-blur-xl transition-all hover:border-green-500/30 hover:text-slate-800 dark:hover:text-white"
          >
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold text-slate-900 dark:text-white sm:text-3xl">{project.name || project.projectName}</h1>
        ...
        {isAdmin && (
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#1e293b] px-5 py-2.5 text-sm font-medium text-slate-700 dark:text-white transition-all hover:border-green-500/30 hover:bg-slate-50 dark:hover:bg-[#1e293b]/80 shadow-sm"
          >
      ...
      <div className="animate-fade-in-up mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <StatCard label="Overdue" value={analytics.overdue} accent="bg-red-500/10" icon={<svg className="h-5 w-5 text-red-600 dark:text-red-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>} />
      </div>

      <div className="animate-fade-in-up mb-8 rounded-xl border border-slate-200 dark:border-white/5 bg-transparent p-5 backdrop-blur-xl transition-colors duration-300">
        <div className="mb-3 flex items-center justify-between">
          <span className="text-sm font-medium text-slate-800 dark:text-white">Overall Progress</span>
          <span className="text-sm font-semibold text-green-600 dark:text-green-400">{analytics.completionPct}%</span>
        </div>
        <div className="h-2.5 w-full overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700/50">
          <div
        ...
        <div className="mt-3 flex gap-6 text-xs text-slate-500 dark:text-slate-400">
          <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-green-500" /> Completed ({analytics.completed})</span>
          <span className="flex items-center gap-1.5"><span className="h-2 w-2 rounded-full bg-purple-500" /> In Progress ({analytics.inProgress})</span>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        <div className="animate-fade-in-up lg:col-span-2">
          <div className="rounded-2xl border border-slate-200 dark:border-white/5 bg-transparent p-6 backdrop-blur-xl min-h-[340px] transition-colors duration-300">
            <h3 className="text-base font-semibold text-slate-800 dark:text-white mb-4">Project Workspace Tasks</h3>
            {tasks.length === 0 ? (
              <div className="text-center py-16 text-slate-400 text-sm">No tasks assigned or created inside this tracking node yet.</div>
            ) : (
              <div className="space-y-3">
                {tasks.map((task) => (
                  <div key={task._id} className="p-4 rounded-xl border border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-[#0f172a]/40 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all hover:border-slate-300 dark:hover:border-white/10">
                    <div>
                      <h4 className="text-sm font-semibold text-slate-800 dark:text-white">{task.title}</h4>
                      {task.description && <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 truncate max-w-md">{task.description}</p>}
                    </div>
                    <div className="flex items-center gap-3">
                      <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full border tracking-wide uppercase ${
                        String(task.status).toLowerCase() === 'completed' ? 'bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20' :
                        String(task.status).toLowerCase() === 'in-progress' || String(task.status).toLowerCase() === 'in progress' ? 'bg-purple-500/10 text-purple-600 dark:text-purple-400 border-purple-500/20' :
        ...
        <div className="animate-fade-in-up" style={{ animationDelay: '120ms' }}>
          <div className="rounded-2xl border border-slate-200 dark:border-white/5 bg-transparent p-6 backdrop-blur-xl transition-colors duration-300">
            <div className="mb-5 flex items-center justify-between">
              <h3 className="text-base font-semibold text-slate-800 dark:text-white">Team Members</h3>
            ...
            {members.length === 0 ? (
              <div className="py-6 text-center text-sm text-slate-400">No members assigned to this scope</div>
            ) : (
              <div className="space-y-3">
                {members.map((member, idx) => {
                  ...
                  return (
                    <div key={member._id || idx} className="flex items-center gap-3 rounded-xl border border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-[#0f172a]/40 p-3">
                      ...
                      <span className="shrink-0 rounded-full bg-slate-100 dark:bg-white/5 px-2 py-0.5 text-[10px] font-medium capitalize text-slate-500 dark:text-slate-400">
                        {role}
                      </span>
                    </div>
                  );
          ...
          <div className="mt-6 rounded-2xl border border-slate-200 dark:border-white/5 bg-transparent p-6 backdrop-blur-xl transition-colors duration-300">
            <h3 className="mb-4 text-base font-semibold text-slate-800 dark:text-white">Project Timeline</h3>
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-slate-800 dark:text-white">
                    {project.endDate || project.dueDate
                      ? new Date(project.endDate || project.dueDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
                      : '—'}
                  </p>
                </div>
              </div>
